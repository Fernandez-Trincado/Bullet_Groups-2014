...
%quantities that can be infered from the simulation. The first one is
%the sub-structure velocity as a fraction of the host's circular
%velocity, $||\vec{v}||/V_{\rm c, host}$, as a measure of the the
%strength of the merger.  The second onw is $\mu$, which measures the geometry
%(head-on vs. grazing) of each interaction. 
...
%the host halo. This velocity is presented in the left panel in
%physical units and in the right panel as a fraction of the circular
...
%very close to each other.  The universality of this profile also holds
%for the group mass range. extending the result found by
%\citep{Hayashi2006} for cluster mass halos. 

%The median of this distribution is located at $v/V_{\rm
%  c,host}=1.1$. We also note the strong break at $v/V_{\rm
%  c,host}=3.0$ present in the data from the group sample, which
...
from the SL2S sample. From the light distribution, only $34$ objects
($\approx 42\%$) have regular isophotes, $33$ had ellongated
isophotes (hints of merging system), and $13$ ($\approx 16\%$) had a
clear bi-modal light distribution.  
...
$64$ \hkpc and the average is $145\pm 52$\hkpc. Comparing this average
displacement against the integratied distribution for the group sample (left
panel, Figure \ref{fig:displacement}) one would expects between $40\%$
to $80\%$ of DM halos to present those displacements. This fraction is
certainly larger than the $\approx 16\%$ inferred from the
observational sample. 

We have taken the results from the simulation and selected all the
objects in the group sample with a DM-DM displacement less than
$350$\hkpc but larger than $64$\hkpc to find XXX objects in
total. From this subsample we find that XXX have a DM-baryon
displacement larger than $87$\hkpc.  

This rough comparison hints that it is possible to find a large fraction
of multi-modal groups with a DM-baryon displacement in a $\Lambda$CDM
cosmology. A proper comparison must take into account all the
observational uncertainties and biases to derive a stronger prediction from the
simulation, something that is beyond the scope of this Letter. 
